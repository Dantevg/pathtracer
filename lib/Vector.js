class Vector {
	...
	/* STATIC METHODS */
	static negative(v){
		return new Vector( ...v.items.map( val => -val ) )
	}
	static add( a, b ){
		if( b instanceof Vector ){
			return new Vector( ...a.items.map( (val, i) => val + b.items[i] ) )
		}else{
			return new Vector( ...a.items.map( val => val + b ) )
		}
	}
	static subtract( a, b ){
		if( b instanceof Vector ){
	...
	static random(n){
		return new Vector( ...Array.from({length: n}, () => Math.random()) )
	}
